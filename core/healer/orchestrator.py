...
class AdaptOpsOrchestrator:
    def __init__(
        ...
        self.mab = AdaptOpsMAB(alpha=1.0, context_dim=16)
    ...
    def _build_context(self, m: PipelineMetrics, event: AnomalyEvent) -> PipelineContext:
        ...
        return PipelineContext(
            ...
            avg_build_time_mins    = float(min(m.build_duration_secs / 600.0, 1.0)),
            flaky_test_ratio       = float(min(m.flaky_test_count / 20.0, 1.0)),
            queue_depth            = float(min(m.queue_depth / 30.0, 1.0)),
            hour_of_day            = hour,
            is_peak_hours          = peak,
            ...
            anomaly_type_deploy    = float(event.anomaly_type.value == "deployment_stall"),
